...
//! | [`palette`] | **T3.4** — re-export of `components::palette`, the Sheffield palette as a contract: every ink/ground pair, its WCAG ratio computed in Rust, and the ink picker for the profile discs |
...
pub mod model;
pub mod nav;
// T3.4 (moved to components/palette.rs at the wave 3 close): re-exported so
// `tv::palette::` paths keep resolving.
