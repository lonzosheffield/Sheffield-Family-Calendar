...
| 9 | PIN bootstrap, 4-digit implied | **MODIFIED.** **6 digits** (R-23d — 100× keyspace at zero UX cost). No PIN on first run; setup code written to the log and to `<data>\setup-code.txt` — **not shown on the TV** (Boss decision, `docs/HANDOFF.md` T2.1 H-24; wording amended at the QA round 2 close per Q2-01). Server-side enforcement only. Exponential backoff, **no hard lockout** (a lockout on a wall display is a self-inflicted outage). |
