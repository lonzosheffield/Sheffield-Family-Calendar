...
| T1.4 | Profiles + settings + parent PIN: `0003_profiles` (drop both CHECKs → FKs); server fns; **6-digit** PIN, argon2id `=0.6.0`, server-enforced, session token (30 d, HttpOnly/Secure/Lax, HTTPS origin only), exponential backoff no lockout; first-run setup code to log + `<data>\setup-code.txt` + TV. **Replace `tests/db_tests.rs:148-157` with an FK-violation test.** | T1.1, T1.2 | S | FK violation; rename emits `ProfilesUpdated` on a 2nd WS client; 5th/6th profile OK; 10 bad PINs → delays ≥ 2ⁿ ms; privileged fn without session errors |
