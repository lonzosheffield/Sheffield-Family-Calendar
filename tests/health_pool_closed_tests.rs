...
fn init_test_env() -> PathBuf {
    ...
    // HS9 (`docs/BACKLOG.md` B-3): this harness — never the shell — pins the
    // data directory, so nothing in this binary can resolve config to the
    // family's live `%ProgramData%\FamilyHub`.
